...
import { loadUserPostsRequest } from '../../slices/postSlice';
    ...
    const onScroll = () => {
      if (window.scrollY + document.documentElement.clientHeight > document.documentElement.scrollHeight - 500) {
        if (hasMorePosts && !loadPostsLoading) {
          dispatch(loadUserPostsRequest({ userId: id, lastId: mainPosts[mainPosts.length - 1]?.id }));
        }
      }
    };
...
export const getServerSideProps = wrapper.getServerSideProps(store => async ({ req, res, params, ...etc }) => {
  ...
  store.dispatch(loadUserPostsRequest({ userId: params.id, lastId: null }));
